...
def Take_an_object_and_decide(request, transaction):
    if hasattr(transaction.user, 'SubscriptionPlan'):
        ...
        if switch_status == "0":  # simple to simple
            ...
            # time_add = ''
            if current_plan_valid_days <= 0:
                time_add = now + timedelta(days=bought_plan.valid_days)
            else:
                time_add = transaction.user.SubscriptionPlan.end_date  # روز باقی مانده پلن قدیمی
                time_add += timedelta(days=bought_plan.valid_days)  # جمع دو تاش
        ...
        ####################################################################################
            ...
            if left_day_current_plan <= 0:
                end_date = now + timedelta(days=bought_plan.valid_days)
            else:
                end_date = old_plan.end_date  # گرفتن زمان پلن قدیمی
                end_date += timedelta(days=bought_plan.valid_days)  # جمع زمان پلن قدیمی و روز های باقی موندش با پلن خریده شد
...
def subscription_payment_create(request):
    if request.method == 'GET':
        ...
        if payment_way == 'melli':
            ...
            melli_invoice_price = invoice_price*10
            data = PaymentRequestInput(amount=melli_invoice_price, order_id=tracking_code, return_url=call_back_url_melli,
