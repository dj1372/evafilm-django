...
name = 'Mellat API'
identifier = 1
properties = ["evafilm", "74592411", "5920279"]
...
class BMLPaymentAPI(object):
    ...
    def request_pay_ref(self, order_id, price, call_back_address, additional_data=None):
        ...
        try:
            ...
            rid = client.service.bpPayRequest(
                terminalId=self.terminalId, userName=self.userName,
                userPassword=self.userPassword, orderId=order_id,
                amount=price, localDate=local_date,
                localTime=local_time, additionalData=additional_data,
                callBackUrl=call_back_address, payerId=0
            )
            print(rid)
    ...
    def verify_payment(self, order_id, ref_id):
        ...
        try:
            ...
            res = client.service.bpVerifyRequest(
                terminalId=self.terminalId, userName=self.userName,
                userPassword=self.userPassword, orderId=order_id,
                saleOrderId=order_id, saleReferenceId=ref_id
            )
            return True, res
        except WebFault as f:
            return False, f.fault.faultstring
    ...
    def settle_payment(self, order_id, ref_id):
        try:
            client = Client(self.service_address)
            res = client.service.bpSettleRequest(
                terminalId=self.terminalId, userName=self.userName,
                userPassword=self.userPassword, orderId=order_id,
                saleOrderId=order_id, saleReferenceId=ref_id)
            return True, res
    ...
    def check_state(self, order_id, ref_id):
        ...
        try:
            client = Client(self.service_address)
            res = client.service.bpInquiryRequest(
                terminalId=self.terminalId, userName=self.userName,
                userPassword=self.userPassword, orderId=order_id,
                saleOrderId=order_id,  saleReferenceId=ref_id
            )
    ...
    def undo_payment(self, order_id, ref_id):
        ...
        try:
            client = Client(self.service_address)
            res = client.service.bpReversalRequest(
                terminalId=self.terminalId, userName=self.userName,
                userPassword=self.userPassword, orderId=order_id,
                saleOrderId=order_id, saleReferenceId=ref_id
            )
